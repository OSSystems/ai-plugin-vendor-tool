...
def read_lock(path: Path) -> LockData:
    try:
        # `json.loads` returns Any; the lockfile is generated by `write_lock`
        # whose input is `LockData`, so we trust the round-trip shape here.
        data: LockData = json.loads(path.read_text())
        return data
